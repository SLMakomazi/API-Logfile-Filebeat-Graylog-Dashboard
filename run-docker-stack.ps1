# Docker Stack Runner for PowerShell
...
if ($LASTEXITCODE -ne 0) {
    Write-Host "❌ Failed to copy JAR file." -ForegroundColor Red
    exit 1
}
...
if ($test -eq 'y' -or $test -eq 'Y') {
    ...
    try {
        ...
        Write-Host "✅ Created item: $($createResponse.name) with id=$($createResponse.id)" -ForegroundColor Green
        
        # Get all items
        Write-Host "Getting all items..." -ForegroundColor Yellow
        $getItems = Invoke-RestMethod -Uri "http://localhost:8080/items" -Method Get
        Write-Host "✅ Found $($getItems.Count) items" -ForegroundColor Green
        
        Write-Host "✅ API test completed. Check Graylog for logs!" -ForegroundColor Green
    } catch {
        Write-Host "❌ API test failed: $($_.Exception.Message)" -ForegroundColor Red
        Write-Host "🔍 Check if the API is running with: docker-compose logs api" -ForegroundColor Yellow
    }
}
